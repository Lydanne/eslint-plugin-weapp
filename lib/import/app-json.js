...
function loadAppJson(appJsonPath) {
  ...
  const resolved = resolveConfiguredAppJsonPath(appJsonPath);
  if (!resolved) return null;
  if (resolved.error) return resolved;

  const absPath = resolved.appJsonPath;
...
/**
 * 解析规则配置里的 appJsonPath。
 *
 * 除了直接传 app.json，也支持传微信开发者工具的 project.config.json：
 *   project.config.json 所在目录 + miniprogramRoot + app.json
 *
 * @param {string} configuredPath app.json 或 project.config.json 的路径
 * @returns {{ appJsonPath: string } | { appJsonPath: string, error: Error } | null}
 */
function resolveConfiguredAppJsonPath(configuredPath) {
  ...
  if (path.basename(absPath) !== PROJECT_CONFIG_BASENAME) {
    return { appJsonPath: absPath };
  }
