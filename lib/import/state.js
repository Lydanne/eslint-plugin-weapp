...
function createFileState(context, opts = {}) {
  ...
  const options = {
    ...
    //   - 'packageJson'：request 的包名须在 package.json 的运行时依赖字段中声明（默认）
    ...
    // 选 packageJson 作默认，是因为 miniprogram_npm 是"构建 npm"的产物，开发阶段经常还没构建就被误报。
    bareModuleResolution:
      rawOptions.bareModuleResolution === "miniprogramNpm"
        ? "miniprogramNpm"
        : "packageJson",
