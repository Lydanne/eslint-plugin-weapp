...
const BARE_MODULE_RESOLUTIONS = ["auto", "packageJson", "miniprogramNpm"];

/**
 * 允许值：'auto' / 'packageJson' / 'miniprogramNpm'；其它一律归为默认 'auto'。
 * schema 层已做 enum 校验，这里做最后一层兜底，避免 undefined 泄露到下游。
 */
function normalizeBareModuleResolution(value) {
  return BARE_MODULE_RESOLUTIONS.includes(value) ? value : "auto";
}
...
function createFileState(context, opts = {}) {
  ...
  const options = {
    ...
    checks: {
      ...
      // 独立分包内使用 npm 包的检查：独立分包无法访问主包 miniprogram_npm，默认要求使用"该独立分包自己构建的 npm"。
      independentNpm: rawOptions.checks?.independentNpm !== false,
    },
    requireRelativePrefix: rawOptions.requireRelativePrefix !== false,
    // 裸模块名（lodash / @wekit/shared / ...）的合法性判定策略：
    //   - 'auto'（默认）：下面两种判据取或——任一命中就视为合法裸包。
    //   - 'packageJson'：request 的包名须在 package.json 的运行时依赖字段中声明
    //       具体字段：dependencies / peerDependencies / optionalDependencies。
    //       devDependencies 是构建 / 测试 / 类型等开发态工具，不进小程序产物，故不计入。
    //   - 'miniprogramNpm'：request 须能解析到 miniprogram_npm 目录下的实际文件（旧行为）。
    // 默认选 auto：miniprogram_npm 是"构建 npm"产物，开发阶段常没构建；但也存在 monorepo 子包只维护 miniprogram_npm
    // 而不维护 package.json 的场景。auto 兼容两种写法，只要任一判据通过就放行。
    bareModuleResolution: normalizeBareModuleResolution(
      rawOptions.bareModuleResolution
    ),
  ...
  // project.config.json 与 package.json 按开发者工具约定同处小程序项目根目录；
  // 拿不到 projectConfigPath 时（极端情况，比如解析阶段出错）退回 miniprogramRoot，
  // package-json 模块内部会对文件不存在静默返回 null。
  const projectRoot = appJsonResolved.projectConfigPath
    ? path.dirname(appJsonResolved.projectConfigPath)
    : miniprogramRoot;
