...
module.exports = {
  meta: {
    ...
    messages: {
      appJsonNotFound:
        '无法读取小程序配置: "{{appJsonPath}}" ({{reason}})，请检查 weapp2/wxml-import 规则的 projectConfigPath 选项',
      ...
      aliasNotSupported:
        'WXML {{tag}} 的 src 不支持 app.json.resolveAlias（原生只认相对路径或绝对路径 `/`）："{{request}}" 命中别名 "{{alias}}"',
      relativePrefixRequired:
        'WXML {{tag}} 的 src "{{request}}" 必须显式使用 "./" 或 "../" 相对路径，或使用 "/" 绝对路径',
    },
  },
...
function checkRequest(context, state, node, rawRequest, extensions, tag) {
  ...
  const effectiveRequest = normalizeWxmlRequest(rawRequest);
  const resolved = resolveImport(effectiveRequest, {
...
function normalizeWxmlRequest(request) {
  if (hasWxmlPathPrefix(request)) {
    return request;
  }
  return "./" + request;
}

function hasWxmlPathPrefix(request) {
  return (
    request.startsWith("/") ||
    request.startsWith("./") ||
    request.startsWith("../")
