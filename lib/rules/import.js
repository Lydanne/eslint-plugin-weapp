    ...
    schema: [
      {
        ...
        properties: {
          ...
          bareModuleResolution: {
            enum: ["packageJson", "miniprogramNpm"],
          },
...
function checkStatic(context, state, node, request, opts = {}) {
  ...
  if (!isWxs && !isPrefixedRequest(request) && !findMatchingAlias(request, state.aliases)) {
    ...
    if (mode === "packageJson") {
      // 认 package.json 中的依赖声明。miniprogram_npm 是“构建 npm”产物，开发阶段不应用作合法性依据。
      if (isDeclaredBareModule(request, state)) {
        return; // 整条静默：不再走 resolveImport / 存在性 / 分包边界。
      }
      ...
      // requireRelativePrefix 关掉时不报前缀错，但后续 resolveImport 大概率还是会 notResolved。
    } else {
      // miniprogramNpm 模式：保留旧行为——命中时继续走路径存在性 + 分包边界检查。
      if (
