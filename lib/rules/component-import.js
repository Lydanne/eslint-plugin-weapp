...
/**
 * @fileoverview 基于 app.json 校验小程序组件配置文件（app.json / 页面/组件 .json）中的路径字段：
 ...
 *
 * 要求：ESLint 9+ 的 flat config，并且该 组件配置文件以 `language: "json/json"`（或 jsonc/json5）加载。
...
module.exports = {
  meta: {
    ...
    messages: {
      appJsonNotFound:
        '无法读取小程序配置: "{{appJsonPath}}" ({{reason}})，请检查 weapp2/component-import 规则的 projectConfigPath 选项',
      appJsonInvalid: '解析小程序配置 "{{appJsonPath}}" 失败: {{reason}}',
      notResolved: '组件配置路径 "{{request}}" 无法解析到有效文件',
      mainImportSubpackage:
        '主包文件的 组件配置字段不能引用分包 "{{to}}" 中的资源: "{{request}}"',
      crossSubpackage:
        '分包 "{{from}}" 的 组件配置字段不能引用其他分包 "{{to}}" 中的资源: "{{request}}"',
      independentCross:
        '独立分包 "{{from}}" 的 组件配置字段不能引用 "{{to}}" 中的资源: "{{request}}"',
      aliasNotSupported:
        '小程序组件配置文件不支持 app.json.resolveAlias（原生只认相对路径 `./..` 或绝对路径 `/`）："{{request}}" 命中别名 "{{alias}}"',
      relativePrefixRequired:
        '组件配置路径 "{{request}}" 必须显式使用 "./" 或 "../" 相对路径，或使用 "/" 绝对路径',
    },
  },
  ...
  create(context) {
    ...
    return {
      Document(node) {
        // 每个 组件配置文件进入时懒加载一次 state（含 app.json）。
...
function setupFileState(context, documentNode) {
  ...
  const options = {
    ...
    requireRelativePrefix: rawOptions.requireRelativePrefix !== false,
...
function walkRoot(context, state, rootObject) {
  const componentPlaceholders = getComponentPlaceholderNames(rootObject);
  ...
  for (const member of rootObject.members) {
    ...
    switch (key) {
      case "usingComponents":
        visitUsingComponents(context, state, value, componentPlaceholders);
        break;
...
function visitUsingComponents(context, state, value, componentPlaceholders) {
 ...
 *
 * **不走 resolveAlias**：微信原生组件配置（usingComponents / componentGenerics.default）
 * 不识别 app.json.resolveAlias；命中任何 alias 前缀都视为误用。
 */
function checkImport(context, state, node, rawRequest, componentInfo) {
  ...
  if (
    ...
    !hasComponentPathPrefix(rawRequest) &&
    !resolvesMiniprogramNpm(rawRequest, state)
  ) {
    context.report({
      loc: node.loc,
  ...
  const resolved = resolveImport(rawRequest, {
    ...
    // 有意不传 aliases —— 组件配置里 alias 不合法
  });

  reportResolution(context, state, node, rawRequest, resolved, componentInfo);
}
...
function reportResolution(context, state, node, rawRequest, resolved, componentInfo) {
  if (!resolved) {
  ...
  if (!result.allowed) {
    if (isAsyncComponentReferenceAllowed(state, targetPackage, componentInfo)) {
      return;
    }
...
function getComponentPlaceholderNames(rootObject) {
  if (!rootObject || rootObject.type !== "Object") return new Set();

  for (const member of rootObject.members) {
    if (memberKey(member) !== "componentPlaceholder") continue;
    const value = member.value;
    if (!value || value.type !== "Object") return new Set();

    return new Set(
      value.members
        .map((item) => memberKey(item))
...
function isAsyncComponentReferenceAllowed(state, targetPackage, componentInfo) {
  const componentName = componentInfo?.componentName;
  if (typeof componentName !== "string" || componentName === "") return false;
  if (!componentInfo.componentPlaceholders?.has(componentName)) return false;
  if (state.appJson.raw?.lazyCodeLoading !== "requiredComponents") return false;
  if (!state.currentPackage || !targetPackage) return false;

  return state.currentPackage.root !== targetPackage.root;
}
...
function hasComponentPathPrefix(request) {
  return (
    request.startsWith("/") ||
    request.startsWith("./") ||
    request.startsWith("../")
