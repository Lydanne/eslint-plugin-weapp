...
module.exports = {
  meta: {
    ...
    messages: {
      appJsonNotFound:
        '无法读取小程序配置: "{{appJsonPath}}" ({{reason}})，请检查 weapp2/wxss-import 规则的 projectConfigPath 选项',
      ...
      relativePrefixRequired:
        '@import 路径 "{{request}}" 必须显式使用 "./" 或 "../" 相对路径，或使用 "/" 绝对路径',
    },
  },
...
function checkImport(context, state, node, rawRequest) {
  ...
  const effectiveRequest = normalizeWxssRequest(rawRequest);
...
function normalizeWxssRequest(request) {
  if (hasWxssPathPrefix(request)) {
    return request;
  }
  return "./" + request;
}

function hasWxssPathPrefix(request) {
  return (
    request.startsWith("/") ||
    request.startsWith("./") ||
    request.startsWith("../")
