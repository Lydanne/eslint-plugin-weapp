...
function checkDynamic(context, state, registeredPages, dynamic) {
  ...
  const effective = aliased !== null ? aliased : stripped;
  ...
  const targetPage = resolvePagePath(effective, state);
  ...
  const result = canImport(state.currentPackage, target.package);
  if (!result.allowed) {
...
function resolvePagePath(request, state) {
