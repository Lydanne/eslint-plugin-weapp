...
- JS 里 `require` / `import` **不支持 `/` 开头的绝对路径**（官方 `require` 文档明确说明；分包下运行时已规测到被解析到当前文件目录或当前分包根的案例）。
- 裸模块名（`lodash` / `@wekit/shared` / …）需要走"构建 npm"产物 `miniprogram_npm/`，但该目录是**构建后**才生成的；开发阶段直接用它做合法性依据会产生大量假阳性。默认 `bareModuleResolution: 'auto'` 同时接受 `package.json` 声明与 `miniprogram_npm/` 构建产物两种判据，见 [`bareModuleResolution`](#裸模块名判定-baremoduleresolution)。
...
4. **JS 绝对路径（`/xxx`）禁用** — 在 JS/TS 里写 `require('/xxx')` / `import '/xxx'` / `import('/xxx')` / `require.async('/xxx')` 都会被直接报为 `jsAbsolutePathNotSupported`（[见下文](#js-绝对路径为什么被禁)）。
5. **`.wxs` 里的 WXS 模块引用** — 原生 WXS 的 `require` 只能引用 `.wxs` 文件模块，且必须使用相对路径；命中 `resolveAlias` 会报 `aliasNotSupportedInWxs`，写 `/...` 或裸模块名会报 `wxsRequireNotRelative`。
...
| `checks.independentNpm`       | `boolean`  | `true`                                     | 关闭后不再检查"独立分包内使用 npm"。默认开启时，独立分包内的 npm 必须解析到该独立分包自己的 `miniprogram_npm/`（即独立分包单独构建过 npm），否则报 `independentNpmNotSupported` |
| `requireRelativePrefix`       | `boolean`  | `true`                                     | 本地路径必须显式写 `./` / `../`；合法裸包名与 alias 不受影响。`/xxx` 绝对路径不再视为合法前缀，统一由 `jsAbsolutePathNotSupported` 拦截 |
| `bareModuleResolution`        | `'auto' \| 'packageJson' \| 'miniprogramNpm'` | `'auto'`                  | 裸包名合法性判据。默认 `'auto'` 下 `package.json` 运行时依赖声明 或 `miniprogram_npm/` 目录实际存在二选一命中即合法；另两种模式是单判据收紧。详见 [裸模块名判定](#裸模块名判定-baremoduleresolution) |
| `ignorePatterns`              | `string[]` | `[]`                                       | 正则源码数组，命中任一者对该引用完全静默（详见下文）                        |

## 裸模块名判定 bareModuleResolution

裸模块指像 `lodash` / `@wekit/shared` / `lodash/fp` 这类不以 `./` `../` `/` 开头、也未命中 `resolveAlias` 的 request。

本规则支持两种判据：

1. **package.json 声明**：request 的包名在项目根 `package.json` 的 `dependencies` / `peerDependencies` / `optionalDependencies` 中精确匹配（**不看** `devDependencies`——开发态工具链不进小程序产物）。项目根 = `project.config.json` 所在目录，按微信开发者工具约定 `package.json` 与它同级。
2. **miniprogram_npm 构建产物**：resolver 从当前文件所在目录沿目录树向上查找 `miniprogram_npm/<pkg>`，命中即通过。**分包自己的 `miniprogram_npm/`（如 `subA/miniprogram_npm/`）会优先于主包被命中**，小程序实际运行时也遵循这个就近查找规则。

三种模式控制"用哪个判据"：

### `'auto'`（默认）

两判据取或：只要"**package.json 声明**"或"**miniprogram_npm 构建产物**"任一命中，即视为合法裸包，整条静默。

适合绝大多数项目：

- 正在开发、还没构建 npm 时，`package.json` 声明兜底，不会假阳性误报。
- monorepo 子包只维护 `miniprogram_npm/` 不维护 `package.json` 的场景，构建产物兜底，同样能识别。
- 两种写法可以混用，用户心智最轻。

### `'packageJson'`

只看 `package.json` 运行时依赖声明。`miniprogram_npm` 里存在但未声明 → 照样报 `relativePrefixRequired`。适合"所有 npm 必须走 `package.json`"的严格项目。

### `'miniprogramNpm'`（旧行为）

只看 `miniprogram_npm/<pkg>` 是否真实构建。命中时不同于另两种模式——**继续走路径存在性 + 分包边界检查**，保留历史语义。切回它的场景：只依赖 `miniprogram_npm` 产物（比如 monorepo 子包完全不维护 `package.json`），或者想通过 `independentCross` / `crossSubpackage` 对 npm 也做跨界检查。

```js
// eslint.config.js
rules: {
  "weapp2/import": ["error", {
    bareModuleResolution: "miniprogramNpm",
  }],
}
```
...
独立分包是**独立打包、独立运行**的子包，**无法访问主包的 `miniprogram_npm/`**。想在独立分包里用 npm，必须在该独立分包目录下单独"构建 npm"，生成 `<subpackage>/miniprogram_npm/<pkg>`。

本规则在**所有三种模式**下都会执行这项额外检查（除非 `checks.independentNpm: false`）：

- 独立分包内的裸模块 request，`resolver` 会先在本独立分包目录下查找 `miniprogram_npm/<pkg>`，命中即放行。
- 若仅在主包（或其它分包）的 `miniprogram_npm/` 命中、或根本没构建（`package.json` 声明了但谁都没生成产物）→ 报 `independentNpmNotSupported`。

提示文案示例：

```text
独立分包 "subInd" 不能使用 npm 包 "@wekit/shared"。独立分包无法访问主包的 miniprogram_npm；
如需在独立分包中使用 npm，请在该独立分包目录下单独构建 npm，生成 "subInd/miniprogram_npm/@wekit/shared"
```
...
require("/subA/components/foo/foo");           // jsAbsolutePathNotSupported
require("/utils/util");                         // jsAbsolutePathNotSupported
import x from "/not/exist";                    // jsAbsolutePathNotSupported
require("../../subA/components/foo/foo");      // mainImportSubpackage

// miniprogram/subA/pages/a1/a1.js
require("../../../subB/pages/b1/b1");          // crossSubpackage

// miniprogram/subInd/pages/i1/i1.js
require("../../../utils/util");                // independentCross
```
...
```js
// miniprogram/utils/main.wxs
require("/utils/shared.wxs");  // WXS：wxsRequireNotRelative（必须相对）
require("@/utils/shared.wxs"); // WXS：aliasNotSupportedInWxs
require("./util");             // WXS 只按 .wxs 解析，不兜同名 .js

// JS 本地引用默认也必须显式写前缀
require("utils/util");         // relativePrefixRequired：必须写 ./utils/util 或 ../utils/util
```

## JS 绝对路径为什么被禁

[官方 `require` 文档](https://developers.weixin.qq.com/miniprogram/dev/reference/api/require.html) 末尾的参数规范里写得很清楚：

> 需要引入模块文件相对于当前文件的相对路径，或 npm 模块名，或 npm 模块路径。**不支持绝对路径**。

实际踩坑：在分包里对一段形如 `/utils/initObs` 的 `require`，微信运行时会把它解析到当前分包根或当前文件目录下（而不是 miniprogramRoot），然后报：

```text
Error: module 'pages/details/zuoye/utils/initObs.js' is not defined, require args is '/utils/initObs'
```

所以规则不再尝试模拟运行时解析（那个行为本身不稳定），而是直接拦住这类写法，要求改为：

- **相对路径**：`../../utils/initObs`。
- **别名**：配 `app.json` 的 `resolveAlias`，比如 `"@/*": "/*"`，然后写 `require('@/utils/initObs')`。alias 由微信开发者工具在编译阶段展开，不依赖运行时解析。

如果某些特殊场景（比如第三方包源码）真的必须留下 `/xxx` 写法且你确定可跑，通过 [`ignorePatterns`](#ignorepatterns) 单条放行。
