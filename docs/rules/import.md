...
- 裸模块名（`lodash` / `@wekit/shared` / …）需要走"构建 npm"产物 `miniprogram_npm/`，但该目录是**构建后**才生成的；开发阶段直接用它做合法性依据会产生大量假阳性。默认以 `package.json` 的运行时依赖声明作为判据，见 [`bareModuleResolution`](#裸模块名判定-baremoduleresolution)。
...
| `bareModuleResolution`        | `'packageJson' \| 'miniprogramNpm'` | `'packageJson'`                    | 裸包名合法性判据。默认读同级 `package.json` 的运行时依赖字段；设为 `'miniprogramNpm'` 回到旧行为（按 `miniprogram_npm/` 目录实际存在性判定）。详见 [裸模块名判定](#裸模块名判定-baremoduleresolution) |
...
读取小程序**项目根目录**下的 `package.json`，按 request 的包名在其 **运行时依赖** 字段里做精确匹配：

- 匹配字段：`dependencies` / `peerDependencies` / `optionalDependencies`。
- **不看** `devDependencies`——那是构建、测试、类型等开发态工具链，不会进入小程序运行产物。
- 命中即整条静默（不再检查 `miniprogram_npm` 下是否存在、也不走分包边界）。
- 未命中且 `requireRelativePrefix: true` → 报 `relativePrefixRequired`。

"项目根目录" = `project.config.json` 所在目录，按微信开发者工具约定 `package.json` 与它同级。无需任何额外配置。

为什么这是默认：`miniprogram_npm` 是 `npm install` 之后**再手动"构建 npm"**产出的目录，开发阶段经常没构建就开始写代码；如果以它为判据，`@wekit/shared` 这种明明在 `package.json` 里声明过的包会被误报。

### `'miniprogramNpm'`（旧行为）

沿被 lint 文件向上查找 `miniprogram_npm/<pkg>`：

- 找得到 → 继续走路径存在性 + 分包边界检查（独立分包引用主包 `miniprogram_npm` 仍会报 `independentCross`）。
- 找不到 + `requireRelativePrefix: true` → 报 `relativePrefixRequired`。

切回它的场景：你确实只依赖 `miniprogram_npm` 的产物（比如 monorepo 子包里压根不维护 `package.json`），或者想保留对独立分包引用 npm 包的严格校验。
